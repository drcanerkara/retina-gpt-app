...
# =========================
def call_openai_critique(clinical_text: str, images, gemini_opinion: dict):
    """GPT-4o reads Gemini's Round 1 output and critiques it."""
    content = [{
        "type": "text",
        "text": (
            "You are a retina specialist conducting a peer review.\n"
            "Your colleague's assessment of the same retinal images is below.\n"
            "Review it critically against the images and clinical info.\n\n"
            ...
            "IMPORTANT for revision_type:\n"
            "- 'evidence_based': you change diagnosis citing a specific imaging/clinical finding\n"
            "- 'sycophantic': you change without substantive justification\n"
            "- 'maintained': you keep your original view with explicit reasoning\n\n"
            f"Clinical info:\n{clinical_text or '(none provided)'}"
        ),
    }]
    for img in images:
...
def call_gemini_critique(clinical_text: str, images, openai_opinion: dict, max_retries: int = 3):
...
def run_analysis():
    ...
    with st.spinner("💬 Round 2 — Cross-critique..."):
        oa_crit_raw, oa_crit = call_openai_critique(clin, images, gem_r1)
        gem_crit_raw, gem_crit, _ = call_gemini_critique(clin, images, oa_r1)
