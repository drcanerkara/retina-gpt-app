...
<div class="rgpt-section-title"><span></span>Patient Information</div>
''', unsafe_allow_html=True)
...
with col4:
    visual_acuity = st.text_input("Visual acuity", value=st.session_state.visual_acuity,
                                   placeholder="e.g. 20/40, 0.3, CF at 2 m")
with col5:
    primary_symptom = st.text_input("Primary symptom", value=st.session_state.primary_symptom,
                                     placeholder="e.g. blurred vision, metamorphopsia")

col6, col7 = st.columns(2)
with col6:
    duration = st.text_input("Duration", value=st.session_state.duration,
                              placeholder="e.g. acute, 3 days, 2 months")
with col7:
    relevant_history = st.text_input("Relevant history", value=st.session_state.relevant_history,
                                      placeholder="e.g. diabetes, high myopia")

additional_notes = st.text_area("Additional notes", value=st.session_state.additional_notes,
                                  placeholder="Any extra clinical context...", height=72)
